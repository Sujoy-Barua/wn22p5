  ...
  class PairComp {
    ...
    public:
      bool operator()(const Pair_type &pair1, const Pair_type &pair2) const {
        return keyLess(pair1.first, pair2.first);
      }
  };
...
typename Map<K, V, C>::Iterator 
  ...
  while (iter != end) {
    if (!comp(iter->first, k) && !comp(k, iter->first)) {
      return iter;
    }
    ++iter;
  }
...
std::pair<typename Map<K, V, C>::Iterator, bool> 
  ...
  if (found != entries.end()) {
    ...
    Iterator new_iter = entries.insert(val);
    std::pair<typename Map<K, V, C>::Iterator, bool>
      output (new_iter, true);
    return output;
  }
}
